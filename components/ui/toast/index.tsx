...
const toastStyle = tva({
    base: "p-4 m-1 rounded-md gap-1 web:pointer-events-auto shadow-hard-5 border-outline-100 top-safe",
    variants: {
        action: {
            error: "bg-error-800",
...
const toastTitleStyle = tva({
    ...
    parentVariants: {
        variant: {
            solid: "",
            outline: "",
            outline2: "text-typography-700 text-base leading-normal font-semibold",
        },
        action: {
            error: "",
            warning: "",
    ...
    ({ className, variant = "solid", action = "muted", ...props }, ref) => {
        return (
            <Root
                ...
                style={{
                    boxShadow: variant === "outline2" ? shadow.xl : "",
                }}
                {...props}
            />
        );
...
const ToastIcon = () => {
    ...
    return (
        <>
            ...
            {action === "error" && (
                <Ionicons name="close-circle" size={24} className="flex-0 basis-auto color-red-500" />
            )}
        </>
    );
};

export { useToast, Toast, ToastTitle, ToastDescription, ToastIcon };
